...
DOCKERFILE="Dockerfile"  # Default to OpenSUSE version

# Function to show help
show_help() {
    echo "Usage: $0 [OPTIONS]"
    echo "Options:"
    echo "  --ubuntu        Use Ubuntu-based image (default: OpenSUSE)"
...
# Check for Ubuntu flag first
if [[ "$1" == "--ubuntu" ]]; then
    DOCKERFILE="Dockerfile.ubuntu"
    shift  # Remove the --ubuntu argument
fi

# Set image name and other variables after potential Ubuntu selection
IMAGE_NAME="yanwk/comfyui-boot:${CONFIG_DIR}"
[[ "$DOCKERFILE" == "Dockerfile.ubuntu" ]] && IMAGE_NAME="${IMAGE_NAME}-ubuntu"
...
case "$1" in
    --build-only)
        ...
        echo -e "${YELLOW}To save the image, run: $0 $([ "$DOCKERFILE" == "Dockerfile.ubuntu" ] && echo '--ubuntu') --save-image${NC}"
        exit 0
        ;;
    --save-image)
    ...
    --install-wheels)
        if ! is_container_running; then
            ...
            echo -e "${YELLOW}Please start the container first with: $0 $([ "$DOCKERFILE" == "Dockerfile.ubuntu" ] && echo '--ubuntu') --load-image${NC}"
            exit 1
        fi
...
echo -e "${YELLOW}To install pre-compiled wheels, run: $0 $([ "$DOCKERFILE" == "Dockerfile.ubuntu" ] && echo '--ubuntu') --install-wheels${NC}"
